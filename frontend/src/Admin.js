...
export default function Admin() {
  ...
  const fetchData = async () => {
    try {
      const [callsRes, statsRes] = await Promise.all([
        fetch(`${BACKEND}/api/admin/calls`, { headers: { password: ADMIN_PASSWORD } }),
        fetch(`${BACKEND}/api/admin/stats`, { headers: { password: ADMIN_PASSWORD } })
      ]);
  ...
  const exportToCSV = () => {
    const headers = ['Name', 'Phone Number', 'Query Type', 'Status', 'Duration (sec)', 'Source', 'Date & Time'];
      ...
      <div style={{ minHeight: '100vh', background: '#f0f2f8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ background: '#fff', borderRadius: 20, boxShadow: '0 8px 40px rgba(0,0,0,0.12)', overflow: 'hidden', display: 'flex', width: 480 }}>
          <div style={{ background: 'linear-gradient(160deg,#1a237e,#0d1257)', padding: '40px 28px', width: 180, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11, lineHeight: 1.6 }}>Restricted access only.</p>
          </div>
    ...
    <div style={{ minHeight: '100vh', display: 'flex', background: '#f0f2f8' }}>
      ...
      <div style={{ background: '#0d1257', width: 200, padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ textAlign: 'center', paddingBottom: 20, marginBottom: 16, borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
          ...
          <p style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: 1, textTransform: 'uppercase', marginTop: 6, marginBottom: 0 }}>Admin Panel</p>
        </div>

        {[
          ['📊', 'Dashboard', null],
          ['📋', 'Refresh Calls', fetchData],
          ['⬇️', 'Export CSV', exportToCSV],
        ].map(([icon, label, action]) => (
          <div key={label} onClick={() => action && action()}
            style={{ padding: '9px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600, color: label === 'Dashboard' ? '#ffb74d' : 'rgba(255,255,255,0.7)', background: label === 'Dashboard' ? 'rgba(255,111,0,0.2)' : 'transparent', cursor: action ? 'pointer' : 'default', display: 'flex', alignItems: 'center', gap: 8 }}>
            {icon} {label}
          </div>
        ))}
      ...
      <div style={{ flex: 1, padding: 24, overflowX: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 20 }}>
            ...
            {stats.byType && stats.byType.map(item => (
              <div key={item._id} style={{ background: '#fff', borderRadius: 12, padding: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                <div style={{ fontSize: 28, fontWeight: 800, color: '#ff6f00', lineHeight: 1 }}>{item.count}</div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <div style={{ padding: '14px 18px', borderBottom: '1px solid #f3f4f6' }}>
            <div style={{ fontWeight: 800, fontSize: 14, color: '#1a237e' }}>All Call Records</div>
            <div style={{ fontSize: 10, color: '#6b7280' }}>Newest first · Includes name, query, duration</div>
          </div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr style={{ background: '#1a237e' }}>
                  {['Name', 'Phone Number', 'Query Type', 'Status', 'Duration', 'Source', 'Date & Time'].map(h => (
                    <th key={h} style={{ padding: '10px 14px', textAlign: 'left', color: 'rgba(255,255,255,0.85)', fontWeight: 600, fontSize: 10, letterSpacing: 0.5, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {calls.length === 0 ? (
                  <tr><td colSpan={7} style={{ textAlign: 'center', padding: 32, color: '#9ca3af' }}>No calls yet</td></tr>
                ) : calls.map((call, i) => (
                  <tr key={call._id} style={{ background: i % 2 === 0 ? '#fff' : '#fafbff', borderBottom: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '11px 14px', color: '#1a237e', fontWeight: 700 }}>{call.name || 'N/A'}</td>
                    <td style={{ padding: '11px 14px', color: '#374151' }}>{call.phoneNumber}</td>
                    <td style={{ padding: '11px 14px', color: '#374151' }}>{call.queryType}</td>
                    <td style={{ padding: '11px 14px' }}>
                      <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 10, fontWeight: 700, background: call.callStatus === 'calling' ? '#dcfce7' : '#fef9c3', color: call.callStatus === 'calling' ? '#166534' : '#854d0e' }}>
                        {call.callStatus}
                      </span>
                    </td>
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 10, fontWeight: 700, background: call.source === 'bulk' ? '#e8eaf6' : '#fff3e0', color: call.source === 'bulk' ? '#1a237e' : '#e65100' }}>
                        {call.source || 'single'}
                      </span>
                    </td>
                    <td style={{ padding: '11px 14px', color: '#6b7280', whiteSpace: 'nowrap' }}>{new Date(call.createdAt).toLocaleString('en-IN')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
